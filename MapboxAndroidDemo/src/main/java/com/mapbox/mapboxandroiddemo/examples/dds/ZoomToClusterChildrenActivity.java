...
public class ZoomToClusterChildrenActivity extends AppCompatActivity implements MapboxMap.OnMapClickListener {
  ...
  private String TAG = "ZoomToClusterChildrenActivity";
  ...
  public boolean onMapClick(@NonNull LatLng point) {
    ...
    if (mapClickFeatureList.size() > 0) {
      FeatureCollection clusterChildrenFeatureCollection = geoJsonSource.getClusterChildren(mapClickFeatureList.get(0));
      moveCameraToChildrenBounds(clusterChildrenFeatureCollection);
      /*if (clusterChildrenFeatureCollection.features().size() > 1) {
        moveCameraToChildrenBounds(clusterChildrenFeatureCollection);
      } else {
        Toast.makeText(this, "another cluster beneath", Toast.LENGTH_SHORT).show();
//        moveCameraToChildrenBounds(geoJsonSource.getClusterChildren(mapClickFeatureList.get(1)));
      }*/
    }
  ...
  private void moveCameraToChildrenBounds(FeatureCollection featureCollectionToInspect) {
    ...
    if (featureCollectionToInspect.features() != null) {
      ...
      Log.d(TAG, "moveCameraToChildrenBounds: latLngList.size() = " + latLngList.size());
      if (latLngList.size() > 1) {
        ...
        Toast.makeText(this, "latLngList.size() =< 1", Toast.LENGTH_SHORT).show();
      }
    }
  }
